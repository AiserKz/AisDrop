...
export default function Item({ item, onClose }) {
    ...
    const [selectSendMessage, setSelectSendMessage] = useState(false);
    ...
    if (!item) {
        return (
            <div>
                ...
                <button onClick={onClose} className="text-2xl text-[#b2b2e5]  flex items-center justify-center bg-[#23232b] rounded-full p-4 transition m-2">
    ...
    const submitMessage = () => {
        axios.post(route('messages.set'), data);
        setSelectSendMessage(false);
        setShowContact(false);
    }

    const selectCategory = () => {
        router.get(route('home'), { category_id: item.category_id }, {
            preserveScroll: true,
            preserveState: true,
        });
    }
    ...
    return (
        <div>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 mb-6">
                ...
                <button
                    onClick={onClose}
                    className="text-[#b2b2e5] hover:text-white bg-[#23232b] rounded-full w-10 p-2 transition border border-[#2c2c35]"
                    aria-label="Назад"
                    >
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
                ...
                <nav className="text-base text-[#b2b2e5] flex flex-wrap items-center gap-x-2 gap-y-1 overflow-hidden text-ellipsis whitespace-normal break-words">
                    <a onClick={onClose} className="hover:text-[#fafafa] truncate max-w-[150px] cursor-pointer">Главная</a>
                    <span>/</span>
                    <button onClick={selectCategory} className="hover:text-[#fafafa] truncate max-w-[150px]">{category}</button>
                    <span>/</span>
                    ...
                    <div className="max-w-4xl mx-auto px-6">
                        <div className="flex flex-col md:flex-row gap-10 items-center md:items-start mb-10">
                            ...
                            <div className="flex-1">
                                ...
                                <Link href={route('user.show', { id: item.user?.id }) + `?from_item=${item.id}`} className="flex items-center gap-2 mt-2">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            {user && (
                                <button
                                    ...
                                    onClick={() => setSelectSendMessage(!selectSendMessage)}
                                >
                            ...
                            {selectSendMessage && (
                                <div className="mt-4"> 
                                    <textarea onChange={e => setData({ ...data, message: e.target.value })} name="message" id="" cols="30" rows="6" placeholder='Ваше сообщение' className="bg-[#23232b] border border-[#2c2c70] rounded-lg p-2 text-[#b2b2e5] resize-none"></textarea>
                                    <button onClick={submitMessage} className="mt-4 w-full bg-[#b2b2e5] hover:bg-[#a2a2b3] text-[#121218] font-bold py-3 rounded-xl text-lg transition flex items-center justify-center gap-2">Отправить</button>
                                </div>
                            )}
                        </div>
                    </div>
